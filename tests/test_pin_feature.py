# test_pin_feature.py — rewritten:

"""Acceptance tests for the pin/unpin feature."""


def _seed(client, app, n=2):
    app.notes.clear()
    for i in range(n):
        client.post("/notes/new", data={"title": f"Note {i}", "body": f"Body {i}"})
...
def test_existing_notes_without_is_pinned_still_render(client, app):
    app.notes.clear()
    app.notes.append({"title": "Legacy", "body": "Old note", "tags": [], "id": 0})
    r = client.get("/")
